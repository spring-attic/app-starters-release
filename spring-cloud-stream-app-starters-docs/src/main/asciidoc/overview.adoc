...
This is useful if you want to deploy the artifacts locally or patch the starter with some customizations or missing features and build again.
...
`spring-cloud-stream-binder-rabbit`. Say, if you have a custom binder you created for some middleware (say Hazelcast),
...
Spring Cloud Stream App Starters are individual repositories hosted under the GitHub organization - spring-cloud-stream-app-starters - on github (http://github.com/spring-cloud-stream-app-starters) +
...
. What are the artifacts that the parent artifact (`app-starters-build`) include from the core bom to be part of all the starters by default and there by making them part of the generated apps? +
* spring-cloud-stream
...
generated apps have `spring-boot-starter` as the parent. Where is the mismatch? +
...
However, this is not the case for the generated apps at runtime. Their parent is `spring-boot-starter` that imports `spring-boot-dependencies` bom that includes a majority of the components that these apps need.
The question then is what if the starter used an artifact managed through the `app-starter-build` or `spring-cloud-build`. Remember that the
`core-app-starter-dependencies` bom is also included in the generated apps. This bom also imports the `spring-cloud-dependencies` bom. This way, at runtime
the apps have access to everything that it used at compile time.

.  Why is there an app starter specific BOM in each app starer repositories? For example, `time-app-dependencies`. +
This is an important BOM. At runtime, the generated apps get the versions used in their dependencies through a BOM that is manaing the dependencies. Since all the boms
...
In your starter you already have the configuration specified for the plugin from the parent. Override the names accordingly. Refer to an exsiting starter for guidelines.
If your app is straightforward, all you have to change is the name of the apps in the <generatedApps> section, and the app-starter specific BOM maven coordinates.
...
You can also override the boot version more permanently by overriding the following property in your starter pom.
    ...
    <bootVersion>new version</bootVersion>
